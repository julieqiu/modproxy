// Package modproxy provides a client for interacting with a proxy.
package modproxy
...
func (c *Client) Info(ctx context.Context, modulePath, requestedVersion string) (_ *versionInfo, err error) {
	defer func() {
		...
		derrors.Wrap(&err, "proxy.Client.Info(%q, %q)", modulePath, requestedVersion)
	}()
...
func (c *Client) Versions(ctx context.Context, modulePath string) (_ []string, err error) {
	...
	escapedPath, err := module.EscapePath(modulePath)
	if err != nil {
		return nil, fmt.Errorf("module.EscapePath(%q): %w", modulePath, errInvalidArgument)
	}
